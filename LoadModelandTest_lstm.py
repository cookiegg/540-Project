...
ytest=xtest_start[:,:,1]
#ytestref=setpoint*np.ones(shape=(time_steps,1))
ykstack=np.zeros(shape=(time_steps,1)) #array which has predicted values

ytestref=setpoint*np.ones(shape=(12 ,1))
#ytestref=ytestref.reshape(1,len(xtest_start[0]),1)
...
for i in range(1,time_steps+1): #Predicting next 100 points with lstm and model
    ...
    xtest_start=np.column_stack((xtest_start,ytestref))
    
    xtest_start=xtest_start.reshape(1,len(xtest_start),3)
