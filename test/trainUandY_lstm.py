...
def get_data(path_to_dataset="./data/manySines.txt", sequence_length=13, ratio=0.995, path_to_test = 0):
    ...
    # Get rid of the X and Y's for targets, keep only the U's
...
# Get the data
lstm_length = 10;
[X_train, y_train, X_test, y_test, NN_train, NN_test]=get_data("./data/randomJumps.txt", lstm_length, 1, "./data/testFile.txt")
# X_train = X_train[:, :, 0:2]
# X_test = X_test[:, :, 0:2]
...
#
# save_model(model,"./savedModels/modelX","./savedModels/modelX")
model = load_model("./savedModels/modelX","./savedModels/modelX_weights")
...
utest_start=X_test[0,:,:] #taking first x points in the test set to start prediction
...
for i in range(1,time_steps+1): #Predicting next 100 points with lstm and model
     ...
     uk_3=ustack[0,lstm_length-4,0]
     yk=nntest_start[0,1]

     yk1=0.6*yk+1.2*0.05*uk_3
     ykstack[i-1]=yk1

     utest1=ustack[0,1:ustack.shape[1],0] #updating recent points
